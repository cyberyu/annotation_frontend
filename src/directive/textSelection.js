const vueSelection = {
  bind (el, binding, vnode) {
    ...
    function handleMouseUp () {
      ...
      var rtn = handleRange()
      rtn.fixStr && binding.value.getSelection && binding.value.getSelection(rtn)
    }
    /**
     * handle range fix
     */
      ...
      if (selection.rangeCount > 0) {
        ...
        if (CheckIntersection(range, region)) {
          ...
          if (binding.modifiers.fix) {
            ...
            rtn.start = range.startOffset
            rtn.end = range.endOffset
          } else {
            var copyRange = range.cloneRange()
            fixRange(copyRange, region)
            rtn.fixStr = copyRange.toString()
            rtn.start = copyRange.startOffset
            rtn.end = copyRange.endOffset
            copyRange.detach()
          }
        }
      }
